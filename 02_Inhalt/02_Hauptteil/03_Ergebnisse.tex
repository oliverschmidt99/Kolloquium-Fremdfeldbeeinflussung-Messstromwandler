...
\begin{frame}{Versuchsdurchführung: Lastprofil}

    \begin{center}
        \includegraphics[height=0.75\textheight, width=0.95\textwidth, keepaspectratio]{03_Ressourcen/Bilder/verlauf_gesamt.png}
    \end{center}

    \slidetime{08:50}{09:40}
    \keynote{
        \begin{itemize}
            \item Stufen: 5\% bis 120\%
            \item Haltezeiten für Stationarität
            \item Automatisierter Zyklus
        \end{itemize}
    }
    \note{
        \begin{itemize}
            \item \textbf{Diagramm:} Zeigt den zeitlichen Verlauf der Laststromstärke (y-Achse in \%) über die aufgezeichneten Datenpunkte (x-Achse).
            \item \textbf{Messpunkte:} Die definierten Stufen bei 5\%, 20\%, 50\%, 80\%, 90\%, 100\% und 120\% decken den relevanten Arbeitsbereich ab.
            \item \textbf{Stationarität:} Die Plateaus ("Haltezeiten") stellen sicher, dass Einschwingvorgänge abgeklungen sind, bevor die Messwerte für den Vergleich herangezogen werden.
            \item \textbf{Überlast:} Die Stufe bei 120\% prüft das Sättigungsverhalten oberhalb des Nennstroms.
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Parallel vs. Dreieck: Verbesserung}
    ...
    \keynote{
        \begin{itemize}
            ...
            \item \textbf{Kompensierte} Varianten: geringere, teils negative Effekte \(\rightarrow\) primärer Hebel bleibt die Kompensation Anordnung
        \end{itemize}
    }
    \note{
        \begin{itemize}
